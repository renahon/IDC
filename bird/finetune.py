...
def train():
    ...
    valid_set_P = get_dataset(
        os.path.join(args.data_path, "val.json"), images, "val", "P"
    )
    ...
    valid_loader = get_dataloader(
        valid_set, args.batch_size, is_train=False, self_define=True
    )

    test_set = get_dataset(os.path.join(args.data_path, "test.json"), images, "test")
    test_loader = get_dataloader(
        test_set, args.batch_size, is_train=False, self_define=True
    )
    ...
    if args.se_block > 1:
        for k, v in model.named_parameters():
            if ("single_img_encoder.layers." in k) and (
                str(args.se_block - 1) not in k
            ):
                v.requires_grad = False  # 固定single encoder和double encoder除了最后一层外的参数
    if args.de_block > 1:
        for k, v in model.named_parameters():
            if ("double_img_encoder.layers." in k) and (
                str(args.de_block - 1) not in k
            ):
    ...
    while True:
        ...
        for step, batch in tqdm(enumerate(train_loader)):
            ...
            writer.add_scalar(
                "lr", optim.state_dict()["param_groups"][0]["lr"], global_step
            )
            optim.step()
            ...
            if global_step > 0 and global_step % args.report == 0:
                ...
                print(
                    "global_step: %d, epoch: %d, report_loss: %.3f, time: %.2f"
                    % (
                        global_step,
                        global_step // len(train_loader),
                        report_loss / n_samples,
                        time.time() - start_time,
                    )
                )
                train_logger.print_train_stats(
                    "mlm",
                    global_step // len(train_loader),
                    global_step,
                    report_loss / n_samples,
                ...
                writer.add_scalar(
                    args.dataset + "_train_loss", report_loss / n_samples, global_step
                )
                ...
                if global_step > 2000 and score > best_score:
                    ...
                    save_model(
                        os.path.join(checkpoint_path, "best_checkpoint.pt"), model
                    )
                else:
                    no_beat += 1
...
def validate(loader, model, global_step):
    ...
    val_log = {"mlm loss": val_loss, "mlm acc": acc}
    return val_log
...
def validate_accumulate(
    ...
    model.eval()
    acc_dict = {"img1": None, "img2": None, "nb_elts": 0}
    img_toklen = Img1.size(1)+1

    with torch.no_grad():
        for i, batch in enumerate(loader):
            acc_dict = model(batch, compute_loss=False, accumulation_dict=acc_dict)
...
def Inference(loader, test_loader, model, global_step):
    ...
    with open(
        os.path.join(val_sent_path, "iter_{}.json".format(global_step)),
        "w",
        encoding="utf-8",
    ) as fout:
...
def test():
    ...
    if args.best_ckpt == "":
        print(
            " ==> test stage load checkpoint from ",
            os.path.join(out_path, "checkpoint/best_checkpoint.pt"),
        )
...
def val():
    ...
    val_loader = get_dataloader(
        val_set, args.batch_size, is_train=False, self_define=True
    )
    ...
    with torch.no_grad():
        with open(
            os.path.join(out_path, "val_result.json"), "w", encoding="utf-8"
        ) as fout:

...
class Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.split == 'train' or self.set_type == 'P':
            ...
            ImgId = data['img']
            cap, cap_len, cap_label, attn_mask = self.padding(description)
            batch['img1'] = img1
            batch['img2'] = img1
            batch['cap'] = cap
            batch['cap_label'] = cap_label
            batch['text_attn_mask'] = attn_mask
            return batch
        # valid and test (used for Inference algorithm)
            ...
            return img1, img1, gt_caps, ImgId
    ...
    def padding(self, sent):
        ...
        text = text + [self.PAD]*PAD_len
        # mask PAD text
        attn_mask = np.ones(length, dtype='uint8')
        if PAD_len > 0:
            attn_mask = np.concatenate((attn_mask, np.zeros(PAD_len, dtype='uint8')))
        output_label = output_label + [-1]*PAD_len
        T = torch.LongTensor(text)
        output_label = torch.LongTensor(output_label)
        return T, length, output_label, attn_mask

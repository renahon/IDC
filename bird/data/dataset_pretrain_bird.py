...
class Dataset(torch.utils.data.Dataset):
    ...
    def filter_data(self):
        '''filter data cannot used for itm_b task(change img1 and img2 order) in this dataset '''
        print("filter data for itm_b task ...")
        datas = self.datas
        self.datas = []
        filter_num = 0
        for jterm in datas:
            ...
            if self.dataset == 'bird': 
                if ('similar' in description) or ('same' in description) or (('no' in description) and ('differences' in description)):
                    filter_num += 1
                else:
                    self.datas.append(jterm)
            elif self.dataset == 'NLVR':
                if ('left' in description) or ('right' in description):
                    self.datas.append(jterm)
                else:
                    filter_num += 1
        print('dataset:', self.dataset, ';  Total datas ', len(self.datas), ';  drop ', filter_num, ' data')
        ...
        if self.task == 'itm_a':
            ...
            if rep_prob < 0.5:
                ...
                if old_idx == idx:
                    ...
                    if tri_prob < 1/3:
                        if self.datas[idx]['img1'] != self.datas[old_idx]['img1']:
                            img1 = torch.from_numpy(self.images[self.datas[idx]['img1']])
                            img1 = img1.view(dim, -1).transpose(0,1)
                            ImgId = self.datas[idx]['img1']+'_'+data['img2']
                        else:
                            # choose the same img1
                            align_label = 1
                    # 1/3 prob replace img2
                    elif tri_prob < 2/3:
                        if self.datas[idx]['img2'] != self.datas[old_idx]['img2']:
                            img2 = torch.from_numpy(self.images[self.datas[idx]['img2']])
                    ...
                    # 1/3 prob replace text
                        ...
                        if (self.datas[idx]['img1'] == self.datas[old_idx]['img1']) and (self.datas[idx]['img2'] == self.datas[old_idx]['img2']):
                            align_label = 1
                        else:
